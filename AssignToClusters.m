function nearest_means = AssignToClusters(im_array, mean_vals)
    ...
    % negative numbers and or numbers in excess of 255 which we will
    ...
    % Preallocate an array used to store each calculation
    nearest_means = zeros(im_rows, im_cols, k_rows);
    ...
    % that 'k' mean
    for i = 1:k_rows      
        nearest_means(:,:,i) = (im_array(:,:,1) - mean_vals(i,1,1)).^2 + ...
    ...
    % case there are two ore more values that are equal, the nearest_mean 
    % will be the layer starting from 1 that the minimum value first 
    % occured on.   
    [~, nearest_means] = min(nearest_means, [], 3);
